...
void ripemd160_process(ripemd160_state *self, const void *void_p, size_t length)
{
    ...
    while (length > 0) {
        ...
        if ((unsigned long) length >= bytes_needed) {
            ...
            self->bufpos += (uint8_t)(bytes_needed);
            self->length += (uint8_t)(bytes_needed << 3);    /* length is in bits */
            p += bytes_needed;
        ...
        self->bufpos += (uint8_t)length;
        self->length += length << 3;    /* length is in bits */
        return;
    }
}

void ripemd160_done(ripemd160_state *self, void *void_out)
{
    unsigned char *out;
    out = void_out;
    /* Append the padding */
...
void simple_ripemd160(const void* in, size_t length, void* out)
{
  ripemd160_state md;
  ripemd160_init(&md);
